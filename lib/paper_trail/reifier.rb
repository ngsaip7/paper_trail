module PaperTrail
  ...
  class Reifier
    ...
    # @api private
    def reify
      return nil if @version.object.nil?
      ...
      model.class.reflect_on_all_associations(:has_one).each do |assoc|
        if assoc.klass.paper_trail_enabled_for_model?
          version = model.class.paper_trail_version_class.joins(:version_associations).
            ...
            where("created_at >= ? OR transaction_id = ?", options[:version_at], @version.transaction_id).
    ...
    def reify_has_many_through(associations, model, options = {})
      associations.each do |assoc|
        ...
        # versions are for those that have been removed from the live
        # associations.
        collection += versions.values.map { |version|
          version.reify(options.merge(:has_many => false, :has_one => false))
        }
    ...
    # Returns true if there's an `IdentityMap`, which was added in rails 3.1.0
    # (https://github.com/rails/rails/pull/76). Every known version of
    # IdentityMap responds to `#without` so the `#respond_to?` isn't strictly
    # necessary, but it's a nice bit of future-proofing.
    def without_identity_map?
      defined?(::ActiveRecord::IdentityMap) &&
        ::ActiveRecord::IdentityMap.respond_to?(:without)
    end

    # In Rails 3.1+, reify confuses the IdentityMap, if enabled. This prevents
    # insertion into the map.
    def without_identity_map(&block)
      if without_identity_map?
